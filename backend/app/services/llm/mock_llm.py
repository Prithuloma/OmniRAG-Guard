...
class MockLLM(BaseLLM):
    ...
    async def generate(
        ...
        *,
        ...
        # Clean query of self-correction / refinement feedback if present
        clean_query = query.split("[REFINEMENT FEEDBACK]")[0].strip()
        query_first_line = clean_query.split("\n")[0].strip()
        ...
        q_words = [w for w in query_first_line.split() if len(w) > 3]
        ...
        if is_summary:
            ...
            answer = (
                ...
                f"## Key Topics\n"
                f"- **Core Theme**: Focused on addressing the user prompt: *{clean_query}*\n"
            ...
            answer = (
                ...
                f"### Key Details\n"
                f"- **Core Topic**: This addresses the question: *{clean_query}*\n"
                f"- **Source Reference**: The context confirms this assertion."
            )
        else:
            answer = (
                f"### Retrieval Assessment\n"
                f"Based on the retrieved documents, the available context does not contain a clear answer to: *{clean_query}*.\n\n"
                f"### Key Details\n"
                f"- **Inquiry**: {clean_query}\n"
                f"- **Status**: Low matching evidence in database."
            )

        # Check if query requests an image/visual
        image_keywords = ["image", "draw", "picture", "diagram", "chart", "illustration", "visual", "sketch", "plot"]
        if any(k in q_lower for k in image_keywords):
            import urllib.parse
            img_prompt = urllib.parse.quote(query_first_line)
            answer += (
                f"\n\n### Concept Visualization\n"
                f"Here is a generated diagram/illustration based on your request:\n\n"
                f"![{query_first_line}](https://image.pollinations.ai/prompt/{img_prompt}?width=1024&height=1024&nologo=true)"
            )
